...
def regression_model_report(reference_data,
                            current_data,
                            column_mapping):
    """
...
def _prepare_drift_data(regressor, config):
    """Prépare les données pour le drift une seule fois."""
    ...
    config.current_feb11["prediction"] = regressor.predict(config.current_feb11[features])

    return config.reference_jan11.reset_index(drop=True), config.current_feb11.reset_index(drop=True)


# 1. Pour le rapport de la baseline (seulement la référence)
def get_baseline_performance_report(regressor, column_mapping, config):
    ref_df, _ = _prepare_drift_data(regressor, config)
    report = Report(metrics=[RegressionPreset()])
    report.run(reference_data=None, current_data=ref_df, column_mapping=column_mapping)
    return report


# 2. Pour la performance comparative (Référence vs Current)
def get_regression_performance_report(regressor, column_mapping, config):
    ref_df, curr_df = _prepare_drift_data(regressor, config)
    report = Report(metrics=[RegressionPreset()])
    report.run(reference_data=ref_df, current_data=curr_df, column_mapping=column_mapping)
    return report


# 3. Pour la dérive de la cible (Target Drift)
def get_target_drift_report(regressor, column_mapping, config):
    ref_df, curr_df = _prepare_drift_data(regressor, config)
...
# 4. Pour la dérive des données (Feature Drift)
def get_data_drift_report(regressor, column_mapping, config):
    ref_df, curr_df = _prepare_drift_data(regressor, config)
    report = Report(metrics=[DataDriftPreset()])
    report.run(reference_data=ref_df,
...
if __name__ == "__main__":
    ...
    perf_report = get_baseline_performance_report(
        regressor,
        column_mapping,
        config)
    ...
    perf_report_w1 = get_regression_performance_report(
        regressor,
        column_mapping,
        config)
    ...
    perf_report_w2 = get_regression_performance_report(
        regressor,
        column_mapping,
        config)
    ...
    perf_report_w3 = get_regression_performance_report(
        regressor,
        column_mapping,
        config)
    ...
    perf_report_target_w3 = get_target_drift_report(
        regressor,
        column_mapping,
        config)
    ...
    perf_report_target_w1 = get_target_drift_report(
        regressor,
        column_mapping,
        config)
    ...
    drift_report = get_data_drift_report(regressor, column_mapping, config)
