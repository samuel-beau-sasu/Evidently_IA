...
class DataResult:
    ...
    preds_train: pd.Series = field(default_factory=pd.Series)
    preds_test: pd.Series = field(default_factory=pd.Series)
...
def generate_regression_performance_report(reference_data, current_data, column_mapping):
    """
    ...
    # Run the report
    regression_performance_report.run(reference_data=reference_data.sort_index(), 
...
def drift_model_analysis(regressor, column_mapping, config):
    ...
    regressor.fit(config.reference_jan11[features], config.reference_jan11[config.target])

    # 3. Prédictions pour Janvier (Reference)
    config.reference_jan11['prediction'] = regressor.predict(config.reference_jan11[features])

    # 4. Prédictions pour Février (Current) - Crucial pour le Drift des prédictions !
...
def drift_data_analysis(regressor, column_mapping, config):
    ...
    # 4. Prédictions pour Février (Current) - Crucial pour le Drift des prédictions !
    # On isole la période cible de février
    #current_period = config.current_feb11.loc['2011-02-01 00:00:00':'2011-02-28 23:00:00'].copy()
    current_period = config.current_feb11.copy()
    current_period['prediction'] = regressor.predict(current_period[features])
...
if __name__ == "__main__":
    ...
    X_train, X_test = model_RF(regressor)
    
    # Generate the regression performance report
    regression_report = generate_regression_performance_report(X_train, X_test, column_mapping)
    ...
    perf_report = drift_model_analysis(regressor,column_mapping, config)
    ...
    perf_report_w1 = drift_model_analysis_week(regressor, column_mapping, config)
    ...
    perf_report_w2 = drift_model_analysis_week(regressor, column_mapping, config)
    ...
    perf_report_w3 = drift_model_analysis_week(regressor, column_mapping, config)
    ...
    perf_report_target_w3 = drift_taget_analysis_week(regressor, column_mapping, config)
    ...
    perf_report_target_w1 = drift_taget_analysis_week(regressor, column_mapping, config)
    ...
    add_report_to_workspace(workspace, PROJECT_NAME, "Rapport Performance Janvier", perf_report)
    
    # Ajout du  rapport (Drift week 1, week 2, week 3)
    add_report_to_workspace(workspace, PROJECT_NAME, "Rapport Performance Fevrier S1", perf_report_w1)
    add_report_to_workspace(workspace, PROJECT_NAME, "Rapport Performance Fevrier S2", perf_report_w2)
    add_report_to_workspace(workspace, PROJECT_NAME, "Rapport Performance Fevrier S3", perf_report_w3)
    
    # Ajout du  rapport target Drift ( week 1, week 3)
    add_report_to_workspace(workspace, PROJECT_NAME, "Rapport Performance des cibles Fevrier S1", perf_report_target_w1)
    add_report_to_workspace(workspace, PROJECT_NAME, "Rapport Performance des cibles Fevrier S3", perf_report_target_w3)
    
    
    # Ajout du  rapport (Drift Data)
    add_report_to_workspace(workspace, PROJECT_NAME, "Rapport Drift Février vs Janvier", drift_report)
